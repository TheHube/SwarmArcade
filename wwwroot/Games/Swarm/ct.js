...
ct.inputs.addAction('LeftTurn', [{"code":"keyboard.KeyA"},{"code":"keyboard.ArrowLeft"}]);
ct.inputs.addAction('RightTurn', [{"code":"keyboard.KeyD"},{"code":"keyboard.ArrowRight"}]);
ct.inputs.addAction('MoveForward', [{"code":"keyboard.KeyW"},{"code":"keyboard.ArrowUp"}]);
ct.inputs.addAction('MoveBackward', [{"code":"keyboard.KeyS"},{"code":"keyboard.ArrowDown"}]);
ct.inputs.addAction('FireWeapon', [{"code":"keyboard.Space"},{"code":"pointer.Primary"}]);
...
ct.rooms.templates['Main'] = {
    ...
    width: 1280,
    height: 720,
    objects: JSON.parse('[{"x":641.5,"y":382.5,"opacity":1,"tint":16777215,"scale":{"x":0.25,"y":0.25},"rotation":0,"exts":{},"customProperties":{},"template":"PlayerShip"},{"x":128,"y":128,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"AlienShip"},{"x":1152,"y":128,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"AlienShip"},{"x":640,"y":896,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"WhiteSquare"}]'),
    bgs: JSON.parse('[{"texture":"Space","depth":0,"exts":{"movementX":0,"movementY":0,"parallaxX":0.3,"parallaxY":0.3,"repeat":"repeat","scaleX":1,"scaleY":1,"shiftX":0,"shiftY":0}},{"texture":"Space2","depth":0,"exts":{"movementX":0,"movementY":0,"parallaxX":0.2,"parallaxY":0.2,"repeat":"repeat","scaleX":0.5,"scaleY":0.5,"shiftX":0,"shiftY":0}},{"texture":"Space3","depth":0,"exts":{"movementX":0,"movementY":0,"parallaxX":0.1,"parallaxY":0.1,"repeat":"repeat","scaleX":0.25,"scaleY":0.25,"shiftX":0,"shiftY":0}}]'),
    tiles: JSON.parse('[]'),
    backgroundColor: '#000000',
    
    onStep() {
        /* room Main — core_OnStep (On frame start event) */
{
...
ct.sound.stop('TitleTheme');
ct.sound.stop('EndTheme');
ct.sound.resume('MainTheme')
...
ct.rooms.templates['Title Screen'] = {
    name: 'Title Screen',
    group: 'ungrouped',
    width: 1280,
    height: 720,
    objects: JSON.parse('[{"x":640,"y":384,"opacity":1,"tint":16777215,"scale":{"x":0.66666667,"y":0.82962963},"rotation":0,"exts":{},"customProperties":{},"template":"TitleSwarm"},{"x":640,"y":512,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"NewGameButton"}]'),
    bgs: JSON.parse('[]'),
    tiles: JSON.parse('[]'),
    backgroundColor: '#000000',
    ...
    onCreate() {
        /* room Title Screen — core_OnRoomStart (On room start event) */
{
ct.sound.resume('TitleTheme');

ct.sound.stop('EndTheme');
ct.sound.stop('MainTheme')
...
ct.rooms.templates['End Screen'] = {
    name: 'End Screen',
    group: 'ungrouped',
    width: 1280,
    height: 720,
    objects: JSON.parse('[{"x":640,"y":512,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"NewGameButton"}]'),
    bgs: JSON.parse('[]'),
    tiles: JSON.parse('[]'),
    backgroundColor: '#000000',
    ...
    onCreate() {
        /* room End Screen — core_OnRoomStart (On room start event) */
{
this.scoreLabel = new PIXI.Text('Score: ' + playerScore, ct.styles.get('White'));
this.addChild(this.scoreLabel)

ct.sound.stop('TitleTheme');
ct.sound.resume('EndTheme');
ct.sound.stop('MainTheme')
...
ct.camera.targetX = this.x;
ct.camera.targetY = this.y;

if(ct.room.playerHealth <= 0){
    ct.emitters.fire('BlueExplosion', this.x, this.y);
    ct.rooms.switch('End Screen')
}

}
/* template PlayerShip — core_OnActionDown (OnActionDown event) */

if (ct.actions['MoveForward'].down) {
    let value = ct.actions['MoveForward'].value;
    
this.speed = 6;

this.direction = (this.rotation * 180 / Math.PI)  + 270;

this.Emission.resume();

}
/* template PlayerShip — core_OnActionRelease (OnActionRelease event) */

if (ct.actions['MoveForward'].released) {
...
if (ct.actions['LeftTurn'].down) {
    let value = ct.actions['LeftTurn'].value;
...
if (ct.actions['RightTurn'].down) {
    let value = ct.actions['RightTurn'].value;
...
if (ct.actions['MoveBackward'].down) {
    let value = ct.actions['MoveBackward'].value;
    
this.speed = 3;
this.direction = (this.rotation * 180 / Math.PI)  + 90;

}
/* template PlayerShip — core_OnActionRelease (OnActionRelease event) */

if (ct.actions['MoveBackward'].released) {
...
if (ct.actions['FireWeapon'].pressed) {
    let value = ct.actions['FireWeapon'].value;
    
this.bullet = ct.templates.copy('PlayerBullet', this.x, this.y);

this.bullet.direction = ct.u.pointDirection(this.x, this.y, ct.pointer.x, ct.pointer.y);

this.bullet.rotation = (this.bullet.direction - 270) * Math.PI / 180;



}
/* template PlayerShip — place_collisionTemplate (Collision with a template event) */
{
    const other = ct.place.meet(this, 'EnemyBullet');
    if (ct.templates.valid(other)) {
        ct.room.playerHealth = ct.room.playerHealth - 1;
    }
}

/* template PlayerShip — core_Timer1 (Timer 1 event) */
...
ct.rooms.switch('End Screen')
    ...
    onCreate: function () {
        /* template PlayerShip — core_OnCreate (On create event) */
{
this.speed = 0;

this.direction = (this.rotation * 180 / Math.PI)  + 270;

this.Emission = ct.emitters.append(this, 'BlueExhaust', {
    position: {
        x: 0,
        y: 100
    }
...
/* template EnemyBullet — core_Timer1 (Timer 1 event) */
...
/* template EnemyBullet — place_collisionTemplate (Collision with a template event) */
{
    const other = ct.place.meet(this, 'PlayerShip');
    if (ct.templates.valid(other)) {
        this.kill = true;
    }
}
...
ct.templates.templates["AlienShip"] = {
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    animationFPS: 30,
    playAnimationOnStart: false,
    ...
    texture: "AlienShip",
    onStep: function () {
        /* template AlienShip — core_OnStep (On frame start event) */
{
this.move();

if (this.timer1 <= 0){
    this.timer1 = Math.random() * 6;
}

}
/* template AlienShip — core_Timer1 (Timer 1 event) */

if (this.timer1 > 0 && this.timer1 <= ((false ? ct.deltaUi : ct.delta) / ct.speed)) {
    this.timer1 = 0;
    
this.direction = ct.u.pointDirection(this.x, this.y, ct.camera.x, ct.camera.y) + (Math.random() * 50 - 25);

this.rotation = (this.direction - 270) * Math.PI / 180;

this.bullet = ct.templates.copy('EnemyBullet', this.x, this.y);
...
/* template AlienShip — place_collisionTemplate (Collision with a template event) */
{
    const other = ct.place.meet(this, 'PlayerBullet');
    if (ct.templates.valid(other)) {
        ct.emitters.fire('RedExplosion', this.x, this.y);

ct.templates.copy('AlienShip', (this.x + (Math.random() * 5000) - 2500), this.y + (Math.random() * 5000) - 2500);
ct.templates.copy('AlienShip', (this.x + (Math.random() * 5000) - 2500), this.y + (Math.random() * 5000) - 2500);
...
/* template AlienShip — core_Timer2 (Timer 2 event) */
...
this.kill = true;
    ...
    onCreate: function () {
        /* template AlienShip — core_OnCreate (On create event) */
{
this.speed = 2;
this.direction = 270;
this.scale.x = 0.25;
...
this.Emission = ct.emitters.append(this, 'RedExhaust', {
    position: {
        x: 0,
        y: 100
    }
...
ct.templates.list['AlienShip'] = [];
        
ct.templates.templates["PlayerBullet"] = {
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    animationFPS: 30,
    playAnimationOnStart: false,
    ...
    texture: "Bullet2",
    onStep: function () {
        /* template PlayerBullet — core_OnStep (On frame start event) */
{
this.move();
}
/* template PlayerBullet — place_collisionTemplate (Collision with a template event) */
{
    const other = ct.place.meet(this, 'AlienShip');
    if (ct.templates.valid(other)) {
        this.kill = true;
    }
}
    ...
    onCreate: function () {
        /* template PlayerBullet — core_OnCreate (On create event) */
{
this.speed = 12;
this.direction = 270;
}
...
ct.templates.list['PlayerBullet'] = [];
        
ct.templates.templates["WhiteSquare"] = {
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    animationFPS: 30,
    playAnimationOnStart: false,
    ...
    texture: "WhiteSquare",
    onStep: function () {
        /* template WhiteSquare — core_OnStep (On frame start event) */
{
this.move();

this.scale.x = ct.room.playerHealth;
}

    },
    onDraw: function () {
        /* template WhiteSquare — core_OnDraw (On frame end event) */
{
this.position.x = ct.camera.position.x;
this.position.y = ct.camera.position.y + 325;
}
    ...
    onCreate: function () {
        /* template WhiteSquare — core_OnCreate (On create event) */
{
this.scale.y = 0.05;
this.scale.x = 0.05
}
...
ct.templates.list['WhiteSquare'] = [];
        
ct.templates.templates["TitleSwarm"] = {
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    animationFPS: 30,
    playAnimationOnStart: false,
    ...
    texture: "TitleSwarm",
    onStep: function () {
        /* template TitleSwarm — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.list['TitleSwarm'] = [];
        
ct.templates.templates["NewGameButton"] = {
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    animationFPS: 30,
    playAnimationOnStart: false,
    ...
    onStep: function () {
        /* template NewGameButton — core_OnStep (On frame start event) */
{
this.move();
}
    ...
    onCreate: function () {
        /* template NewGameButton — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
    
if(ct.pointer.hovers(this)){
...
ct.templates.list['NewGameButton'] = [];
...
(function resAddon(ct) {
    ...
    ct.res = {
        ...
        textures: {},
        skeletons: {},
        groups: [{"fonts":{"ungrouped":[]},"textures":{"ungrouped":["Space","Space2","goodShip","Space3","Exhaust","Bullet","Bullet2","AlienShip","Exhaust-modified","WhiteSquare","TitleSwarm","NewGameButton"]},"styles":{"ungrouped":["White"]},"rooms":{"ungrouped":["Main","Title Screen","End Screen"]},"templates":{"ungrouped":["PlayerShip","EnemyBullet","AlienShip","PlayerBullet","WhiteSquare","TitleSwarm","NewGameButton"]},"sounds":{"ungrouped":["TitleTheme","MainTheme","EndTheme"]},"emitterTandems":{"ungrouped":["BlueExhaust","RedExhaust","RedExplosion","BlueExplosion"]}}][0],
        /**
        ...
        loadGame() {
            ...
            const tiledImages = [{"Space":{"source":"./img/t0.png","shape":{"type":"rect","top":0,"bottom":1000,"left":0,"right":1000},"anchor":{"x":0,"y":0}},"Space2":{"source":"./img/t1.png","shape":{"type":"rect","top":0,"bottom":1000,"left":0,"right":1000},"anchor":{"x":0,"y":0}},"Space3":{"source":"./img/t2.png","shape":{"type":"rect","top":0,"bottom":1000,"left":0,"right":1000},"anchor":{"x":0,"y":0}}}][0];
            const sounds = [[{"name":"TitleTheme","wav":false,"mp3":"./snd/m95r98GRTP7rD3.mp3","ogg":false,"poolSize":5,"isMusic":false},{"name":"MainTheme","wav":false,"mp3":"./snd/BNjwrRq6j1mfLr.mp3","ogg":false,"poolSize":5,"isMusic":false},{"name":"EndTheme","wav":false,"mp3":"./snd/P2tc3gt5rj3KHk.mp3","ogg":false,"poolSize":5,"isMusic":false}]][0];
            const bitmapFonts = [{}][0];
